  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/parse-profile', {
        ...
        body: JSON.stringify({ pdfBase64: base64, type: 'resume' })
      })
          ...
          <div className="border-2 border-dashed border-gray-700 rounded-xl p-8 hover:border-purple-500 transition-colors">
            {loading ? (
              <div className="flex flex-col items-center">
                ...
                <p className="text-sm text-gray-400">Extracting details from Resume PDF...</p>
              </div>
            ) : (
              <div className="space-y-2">
                <div className="text-3xl">📝</div>
                <p className="text-sm font-medium text-white">Click to upload Resume PDF</p>
                <p className="text-xs text-gray-500">Upload any resume PDF</p>
              </div>
            )}
          </div>
        </label>
